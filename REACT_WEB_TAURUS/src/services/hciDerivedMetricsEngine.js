...
function calculateCanonicalHciScores(rows, athleteRows) {
  ...
  const pressureScore =
    pressureLoadEvento <= 1.2
      ? limit010(7.0 + Math.abs(pressureLoadEvento - 1.2) * 3.0)
      : limit010(7.0 - (pressureLoadEvento - 1.2) * 3.0)
...
function rhythmScoreFromStd(stdValue) {
  ...
  if (safeValue <= 0.42) return 10.0
  if (safeValue <= 0.5) return 10.0 - ((safeValue - 0.42) / 0.08) * 1.5
  if (safeValue <= 0.9) return 8.5 - ((safeValue - 0.5) / 0.4) * 3.5
  return Math.max(0, 5.0 - (safeValue - 0.9) * 4.0)
}
...
function scoreLevel(score) {
  if (Number(score) >= 9) return 'ELITE'
  if (Number(score) >= 7) return 'ALTO RENDIMENTO'
  if (Number(score) >= 5) return 'INTERMEDIARIO'
  return 'INICIANTE'
}
