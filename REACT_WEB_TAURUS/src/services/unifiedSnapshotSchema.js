...
function ensureAdminLeadAccess(snapshot) {
  const now = Date.now()
  const adminLeads = [
    {
      leadId: 'clynson.admin@local',
      athleteName: 'Clynson',
      athleteEmail: 'clynson.admin@local',
    ...
    {
      leadId: 'clynson@gmail.com',
      athleteName: 'OLIVEIRA, CLYNSON',
      athleteEmail: 'clynson@gmail.com',
      source: 'ADMIN_BOOTSTRAP',
      createdAt: now,
      updatedAt: now,
      subscriptionTier: 'ADMIN',
  ...
  ;(snapshot?.leads || []).forEach((lead) => {
    leadMap.set(lead.leadId || normalizeAthleteName(lead.athleteName), lead)
  })

  adminLeads.forEach((adminLead) => {
    const current = leadMap.get(adminLead.leadId)

    leadMap.set(adminLead.leadId, {
      ...adminLead,
      ...current,
      athleteName: adminLead.athleteName,
      athleteEmail: adminLead.athleteEmail,
      subscriptionTier: 'ADMIN',
      role: 'ADMIN',
      updatedAt: now,
    })
...
function deriveAthletes(snapshot) {
  ...
  ;(snapshot?.leads || []).forEach((lead) => {
    ...
    athleteMap.set(athleteName, {
      ...
      modality: null,
      levelCode: null,
      subscriptionTier: lead.subscriptionTier || 'FREE',
      role: lead.role || 'ATHLETE',
...
export function normalizeSnapshotToUnifiedSchema(snapshot) {
  ...
  const snapshotWithAdminAccess = ensureAdminLeadAccess(snapshot)
  const athletes = deriveAthletes(snapshotWithAdminAccess)
  const approvedIssf = deriveApprovedIssfEntities(snapshotWithAdminAccess)
  const pendingHeaders = derivePendingIssfHeaders(snapshotWithAdminAccess)
  const archivedHeaders = deriveArchivedIssfHeaders(snapshotWithAdminAccess)
  ...
  return {
    ...
    unifiedTargetSessions: deriveUnifiedTargetSessions(snapshotWithAdminAccess),
    unifiedPrescriptions: deriveUnifiedPrescriptions(snapshotWithAdminAccess),
    athleteViewCache: deriveAthleteViewCache(snapshotWithAdminAccess),
    unifiedMetadata: {
      schemaVersion: '1.0',
      nativeLanguage: 'en',
