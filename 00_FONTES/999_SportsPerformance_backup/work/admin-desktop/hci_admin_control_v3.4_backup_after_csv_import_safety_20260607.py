...
# VERSION: 3.4 - Admin TargetScan import and dual-axis Rhythm chart
...
def csv_score_from_token(value, is_rifle):
    ...
    if not (0.0 <= score <= 10.9):
        return None
    return score if is_rifle else float(int(score))


def sniff_csv_rows(text):
    sample = str(text or "").strip().lstrip("\ufeff")
    if not sample:
        return [], []
    try:
    ...
    rows = [row for row in csv.reader(StringIO(sample), dialect) if any(str(cell).strip() for cell in row)]
    headers = rows[0] if rows and any(re.search(r"[A-Za-zÀ-ÿ]", str(cell or "")) for cell in rows[0]) else []
    return headers, rows[1:] if headers else rows


def parse_user_result_csv(text, athlete, event, session, prova):
    raw = str(text or "").strip()
    if not raw:
        raise ValueError("CSV de prova vazio.")
    is_rifle = prova == "RIFLE"
    headers, data_rows = sniff_csv_rows(raw)
    chunks = []
    grouped = {}

    if headers:
        normalized = [normalize_csv_header(header) for header in headers]
        series_names = {"serie", "series", "sr", "serienumero", "numeroserie"}
        value_names = {"valor", "score", "pontos", "pontuacao", "resultado", "tiro", "shot"}
        series_idx = next((i for i, name in enumerate(normalized) if name in series_names), None)
        shot_cols = [
            i for i, name in enumerate(normalized)
            if re.fullmatch(r"(?:t|tiro|shot|disparo|sr)\d{1,2}", name or "")
        ]
        value_idx = next((i for i, name in enumerate(normalized) if name in value_names), None)

        for row in data_rows:
            if shot_cols:
                values = []
                for index in shot_cols:
                    if index < len(row):
                        score = csv_score_from_token(row[index], is_rifle)
                        if score is not None:
                            values.append(score)
                if len(values) >= 10:
                    chunks.append(values[:10])
                elif values and series_idx is not None and series_idx < len(row):
                    grouped.setdefault(str(row[series_idx]).strip() or f"SR{len(grouped)+1}", []).extend(values)
                continue
            if value_idx is not None and value_idx < len(row):
                score = csv_score_from_token(row[value_idx], is_rifle)
                if score is not None:
                    key = str(row[series_idx]).strip() if series_idx is not None and series_idx < len(row) else "ALL"
                    grouped.setdefault(key or "ALL", []).append(score)
                continue
            values = [csv_score_from_token(cell, is_rifle) for cell in row]
    ...
    if not chunks:
        values = [csv_score_from_token(token, is_rifle) for token in re.findall(r"10(?:[,.]\d)?|\d(?:[,.]\d)?", raw)]
        values = [value for value in values if value is not None]
        chunks = [values[start:start + 10] for start in range(0, min(len(values), 60), 10) if len(values[start:start + 10]) == 10]

    if not chunks:
        raise ValueError("Nao encontrei series completas de 10 disparos no CSV.")

    rows = build_target_scan_rows_from_chunks(chunks[:6], athlete, event, session, prova)
    for row in rows:
        row["source"] = "APP_RESULT_CSV"
        row["notes"] = "Importado de CSV de prova enviado pelo usuario via plataforma Admin."
    return rows


def import_result_csv(payload):
    athlete = str(payload.get("athlete", "")).strip()
    event = str(payload.get("event", "")).strip() or "CSV_PROVA_ADMIN"
    session = str(payload.get("session", "")).strip() or "PROVA"
    ...
    rows = parse_user_result_csv(text, athlete, event, session, prova)
    tmp, db_path = copy_app_database()
    try:
        conn = sqlite3.connect(db_path)
        cur = conn.cursor()
        for row in rows:
            cur.execute(
                ...
                """,
                (
                    row["submissionId"], row["athlete"], row["event"], row["session"], row["prova"], row["serie"],
                    row["shots"], row["source"], row["notes"], row["status"], row["submittedAt"], None, None,
                ),
            )
        conn.commit()
        conn.close()
...
<html lang="pt-BR">
<head>
  <meta charset="utf-8" /><title>SportsPerformance Admin v3.4</title>
  <style>
...
<main>
  ...
  <section class="wide"><h2>Visao do atleta <button onclick="window.print()" style="float:right; font-size:11px">Imprimir relatorio</button></h2>
  ...
  <section class="wide no-print"><h2>Importar TargetScan para atleta</h2>
    <div style="display:grid; grid-template-columns:1fr 1fr 1fr 1fr 1fr; gap:8px; margin-bottom:8px">
      ...
      <input id="resultCsvFile" type="file" accept=".csv,.txt" title="CSV de prova enviado pelo usuario" />
    </div>
    <textarea id="targetScanText" placeholder="Cole aqui o texto exportado do TargetScan, ou carregue TXT/CSV. PDF sem extracao textual sera avisado. Para CSV de prova, carregue o arquivo e use o botao CSV ao lado do TargetScan." style="width:100%; min-height:120px; border:1px solid var(--line); border-radius:6px; padding:8px"></textarea>
    <div style="margin-top:8px; display:flex; gap:8px; align-items:center">
      <button onclick="importTargetScan()">Carregar TargetScan</button>
      <button onclick="importResultCsv()" style="background:#0f766e">Carregar CSV de prova</button>
...
<script>
let SNAPSHOT = null; let ATHLETE_TAB = 'Resumo'; let LANG = 'PT'; let TRAINING_LIMIT = 5; let RHYTHM_PICK = {}; let TARGETSCAN_PDF_BASE64 = ''; let RESULT_CSV_READY = false;
...
resultCsvFile?.addEventListener('change', async (event)=>{ const file=event.target.files?.[0]; if(!file) return; TARGETSCAN_PDF_BASE64=''; RESULT_CSV_READY=true; targetScanText.value=await file.text(); targetScanStatus.textContent='CSV de prova '+file.name+' carregado. Use Carregar CSV de prova.'; });
targetScanFile?.addEventListener('change', async (event)=>{ const file=event.target.files?.[0]; if(!file) return; TARGETSCAN_PDF_BASE64=''; RESULT_CSV_READY=false; if(file.type==='application/pdf' || file.name.toLowerCase().endsWith('.pdf')){ const bytes = new Uint8Array(await file.arrayBuffer()); let binary=''; const chunk=0x8000; for(let i=0;i<bytes.length;i+=chunk){ binary += String.fromCharCode(...bytes.subarray(i, i+chunk)); } TARGETSCAN_PDF_BASE64=btoa(binary); targetScanStatus.textContent=`PDF ${file.name} carregado. A extracao textual sera feita no Admin ao importar.`; return; } targetScanText.value=await file.text(); targetScanStatus.textContent=`Arquivo ${file.name} carregado.`; });
async function importResultCsv(){ const athlete=athleteSelect.value; const payload={athlete,event:targetScanEvent.value||'CSV_PROVA_ADMIN',session:targetScanSession.value||'PROVA',prova:targetScanProva.value,text:targetScanText.value}; targetScanStatus.textContent='Importando CSV de prova...'; const res=await fetch('/api/import-result-csv',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const data=await res.json(); if(!data.ok){ targetScanStatus.textContent=data.error||'Falha ao importar CSV de prova.'; return; } targetScanStatus.textContent='CSV importado: '+data.series+' series / '+data.shots+' disparos para '+athlete+'.'; targetScanText.value=''; RESULT_CSV_READY=false; if(resultCsvFile) resultCsvFile.value=''; await load(); }
async function importTargetScan(){ const athlete=athleteSelect.value; const payload={athlete,event:targetScanEvent.value,session:targetScanSession.value,prova:targetScanProva.value,text:targetScanText.value,pdfBase64:TARGETSCAN_PDF_BASE64}; targetScanStatus.textContent='Importando...'; const res=await fetch('/api/import-targetscan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const data=await res.json(); if(!data.ok){ targetScanStatus.textContent=data.error||'Falha ao importar TargetScan.'; return; } targetScanStatus.textContent=`Importado: ${data.series} series / ${data.shots} disparos para ${athlete}.`; targetScanText.value=''; TARGETSCAN_PDF_BASE64=''; RESULT_CSV_READY=false; targetScanFile.value=''; if(resultCsvFile) resultCsvFile.value=''; await load(); }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parsed.path == "/api/import-result-csv":
                return self._send(import_result_csv(body))
