...
# VERSION: 4.0 - Stable core restored from 3.4 (TargetScan import and dual-axis Rhythm chart)
ROOT = os.path.dirname(os.path.abspath(__file__))
ADB = os.path.expandvars(r"%LOCALAPPDATA%\Android\Sdk\platform-tools\adb.exe")
PACKAGE = "com.example.sportsperformance"
PORT = 8767
HTML_LAYOUT_PATH = os.path.join(ROOT, "hci_admin_control_v4_layout.html")
...
def load_v4_layout_html():
    with open(HTML_LAYOUT_PATH, "r", encoding="utf-8") as fh:
        return fh.read()
...
<body>
<header>
  <h1>SportsPerformance Admin Control v4.0</h1>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            try:
                return self._send(load_v4_layout_html())
            except Exception as exc:
                return self._send(f"<h1>Falha ao abrir layout v4</h1><pre>{exc}</pre>", status=500)
...
if __name__ == "__main__":
    ...
    print(f"SportsPerformance Admin v4.0 rodando sob demanda em {url}")
