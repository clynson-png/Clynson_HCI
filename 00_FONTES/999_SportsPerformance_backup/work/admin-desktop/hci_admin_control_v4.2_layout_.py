...
function renderResumo(item) {
    ...
    issfResults.innerHTML = table((SNAPSHOT.shotSeries || []).slice(0,200), [['Atleta','atleta'],['Evento','evento'],['Sessão','sessao'],['Prova','prova'],['Série','serie'],['Tiros','tiros'],['Data','dataColeta',fmtDate]]);
...
async function importTargetScan() { const payload = {athlete:athleteSelect.value, event:tsEvent.value, session:tsSession.value, prova:tsProva.value, text:tsText.value}; const d=await fetch('/api/import-targetscan', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)}).then(r=>r.json()); if(!d.ok) return alert(d.error||'Falha ao importar TargetScan.'); alert(`Importado: ${d.series} séries / ${d.shots || ''} disparos.`); tsText.value=''; await load(); }
