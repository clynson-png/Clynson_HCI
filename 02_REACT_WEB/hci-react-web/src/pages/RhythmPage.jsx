...
function RhythmPage({
  ...
  const competitionReferenceValue = useMemo(
    () => buildCompetitionReferenceValue(sessions),
    [sessions]
  )
  ...
  return (
    <main className="dashboard">
      ...
      <RhythmMainChart
        ...
        referenceValue={competitionReferenceValue}
      />
...
function buildMedianRhythm(sessions, aggregationMode) {
  ...
  return seriesNames.map((serieName) => {
    ...
    return {
      ...
      std: round(average(matchingSeries.map((item) => item.std))),
...
function buildCompetitionReferenceValue(sessions) {
  const competitionSessions = filterSessionsByMode(sessions, 'COMPETITION_MEDIAN')

  const competitionSeriesValues = competitionSessions
    .map((session) => {
      const seriesValues = (session.series || [])
        .map((serie) => average([serie.p1, serie.p2, serie.p3]))
        .filter(isNumber)

      if (!seriesValues.length) {
        const rhythmValues = (session.rhythmPath || [])
          .map((item) => Number(item.val))
          .filter((value) => !Number.isNaN(value))

        return average(rhythmValues)
      }

      return average(seriesValues)
    })
    .filter(isNumber)

  const trend = logarithmicTrend(competitionSeriesValues)
  if (trend) {
    return round(trend(competitionSeriesValues.length + 1))
  }

  return round(average(competitionSeriesValues))
}
...
function average(values) {
  const clean = values.filter(isNumber).map(Number)
  if (!clean.length) return null
