...
function ImportSiusVideoPanel({ onSaveSession, onClose }) {
...
function normalizeDirection(value, weaponClass = modality) {
    ...
    reader.onload = function (event) {
      try {
        ...
        if (!Array.isArray(data.series_table)) {
          setError('HCI IA JSON loaded, but series_table was not found.')
          return
        }
        ...
        setSaveMessage('HCI IA JSON imported and replaced review table')
        setError('')
      } catch (err) {
        console.error(err)
        setError('Invalid HCI IA result JSON')
      }
    }
  ...
  function handleGenerateHciIaPackage() {
    if (!sourceFile) {
      setError('Select a SIUS video, SIUS receipt image, TargetScan file, or PDF first.')
      return
    }
    ...
    setSaveMessage('HCI IA package generated. Send this package and the source file to HCI IA, then import the returned JSON.')
    setError('')
  }
  ...
  setSaveMessage('HCI IA session saved to active snapshot')
}
  ...
  return (
    <section className="panel" style={{ width: '100%' }}>
      <h2>HCI IA IMPORT</h2>
      <p>Universal source package · HCI IA JSON import · Admin editable table</p>

      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', gap: 12, marginTop: 16 }}>
        <input value={athlete} onChange={(e) => setAthlete(e.target.value)} placeholder="Athlete" />
      ...
      {extractedSession && (
        <div style={{ marginTop: 24 }}>
          <h3>HCI IA Review Session</h3>
          <p style={{ fontWeight: 700 }}>FLAG: ADMIN_REVIEW_REQUIRED</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
            <button type="button" onClick={saveReviewedSiusSession}>
              Approve and Save HCI IA Session
            </button>
          </div>
        </div>
      )}
