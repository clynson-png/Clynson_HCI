function RhythmMainChart({ data = [], discipline = 'RIFLE', referenceValue = null }) {
  const width = 980
  const height = 360
  ...
  const hasReferenceValue =
    ...
    !Number.isNaN(Number(referenceValue))
  const resolvedReferenceValue =
    hasReferenceValue ? Number(referenceValue) : null
  ...
  function yLeft(value) {
    return (
      padding.top +
      ((leftMax - Number(value)) / (leftMax - leftMin)) * chartHeight
    )
  }
  ...
  function yRight(value) {
    return (
      padding.top +
      ((rightMax - Number(value)) / (rightMax - rightMin)) * chartHeight
    )
  }
  ...
  function buildScoreLine() {
    return data.map((item) => {
      const values = [item.p1, item.p2, item.p3]
  ...
  const predictionLine = buildLogPrediction(scoreLine)
    ...
    <section className="panel">
      ...
      <div style={{ padding: 16, overflowX: 'auto' }}>
        ...
        >
          ...
          {hasReferenceValue && (
            <>
              <line
                x1={padding.left}
                x2={width - padding.right}
                y1={yRight(resolvedReferenceValue)}
                y2={yRight(resolvedReferenceValue)}
                stroke="#9333ea"
                strokeWidth="2"
                strokeDasharray="6 6"
              />
              ...
              <text
                ...
                y={yRight(resolvedReferenceValue) - 6}
                fontSize="11"
                textAnchor="end"
                fill="#9333ea"
              >
                Reference {resolvedReferenceValue.toFixed(2)}
              </text>
            </>
          )}
          ...
          {data.map((item, index) => {
            ...
            return (
              <g key={`bars-${item.serie}`}>
                <rect
                  x={groupX - stdBarWidth / 2}
                  y={barY(stdValue)}
                  width={stdBarWidth}
                  height={barHeight(stdValue)}
                  fill="#2dd4bf"
                  rx="3"
                />
                <rect
                  x={groupX + stdBarWidth / 2 + markerGap}
                  y={barY(dropValue)}
                  width={markerWidth}
                  height={barHeight(dropValue)}
                  fill="#dc2626"
                  rx="3"
                />
                <rect
                  x={groupX + stdBarWidth / 2 + markerGap + markerWidth + markerGap}
                  y={barY(breakValue)}
                  width={markerWidth}
                  height={barHeight(breakValue)}
                  fill="#f59e0b"
                  rx="3"
                />
              </g>
          ...
          <path
            d={smoothPath(predictionLine, yRight)}
            fill="none"
            stroke="#dc2626"
            strokeWidth="2"
            strokeDasharray="8 6"
        ...
        <div style={{ display: 'flex', gap: 16, paddingTop: 8, flexWrap: 'wrap' }}>
          <span><strong style={{ color: '#2dd4bf' }}>■</strong> STD</span>
          <span><strong style={{ color: '#dc2626' }}>■</strong> Drop Depth</span>
          <span><strong style={{ color: '#f59e0b' }}>■</strong> Break Count</span>
          <span><strong style={{ color: '#2563eb' }}>━</strong> Average of T1:3, T4:7, T8:10</span>
