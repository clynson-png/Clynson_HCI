...
function NewManualSessionPanel({ initialDraft, onSaveSession, onClose }) {
...
function validateManualSession() {
  ...
  if (filledScores !== 60) {
    setValidationMessage(`MISSING SCORES: ${filledScores}/60 filled`)
    return
  }

  if (filledDirections !== 60) {
    setValidationMessage(`MISSING DIRECTIONS: ${filledDirections}/60 filled`)
    return
  }

  if (invalidScores > 0) {
    setValidationMessage(`INVALID SCORES: ${invalidScores}`)
    return
  }

  setValidationMessage('VALID SESSION: 60 scores and 60 directions completed')
}
...
function saveManualSession() {
  ...
  setValidationMessage('SAVED: manual session sent to active snapshot')
}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
        <div>
          ...
          <p style={{ marginTop: 4, opacity: 0.75 }}>
            Admin manual input · 6 series · 60 scores · 60 directions
          </p>
        </div>
...
<div style={{ overflowX: 'auto', marginTop: 18, paddingBottom: 8 }}>
      ...
      >
        ...
        <button type="button" onClick={saveManualSession}>Save</button>
        <button type="button" onClick={onClose}>
          Cancel
        </button>
      </div>
