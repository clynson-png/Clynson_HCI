...
import { buildHappyBlocks } from './happyAnalysisService'
...
export function mapSnapshotToAthleteView(snapshot, athleteName) {
  ...
  const resolvedAthleteName =
    resolveExistingAthleteName(snapshot, athleteName) ||
  ...
  const athlete360 =
    fallbackAthletes.find((item) => athleteNamesMatch(item.athlete, resolvedAthleteName)) || null
  ...
  const happyBlocks = buildHappyBlocks({
    snapshot,
  ...
  return {
    ...
    athleteSelfAnalysis: happyBlocks.athleteSelfAnalysis,

    crossAnalysis: happyBlocks.crossAnalysis,
...
function findAthleteEmail(snapshot, athleteName) {
  const lead = (snapshot?.leads || []).find(
    (item) => athleteNamesMatch(item.athleteName, athleteName)
  )
...
function buildRenderableSessions(snapshot, athleteName) {
  ...
  const athlete360 = (snapshot?.athlete360 || []).find((item) => athleteNamesMatch(item.athlete, athleteName))
  ...
  return headers
    .map((header) => {
      ...
      return {
        ...
        std: roundNumber(calculateStdFromAverage(shotScores)),
...
function buildRenderableSeriesList(snapshot, sessionId) {
  ...
  orderedSeries.forEach((serie) => {
    ...
    result.push({
      ...
      std: roundNumber(calculateStdFromAverage(shots)),
...
function calculateStdFromAverage(values) {
  const clean = (values || [])
    .map((value) => Number(value))
    .filter((value) => !Number.isNaN(value))
  ...
  const baseline = calculateAverage(clean)
  const variance =
