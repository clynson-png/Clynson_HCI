...
# VERSION: 4.2 layout + v3.4 motores conectados - visual preservado
...
<html lang="pt-BR">
<head>
  <meta charset="utf-8" /><title>SportsPerformance Admin v4.0</title>
  <style>
    :root {
...
<body>
<header>
  <h1>Admin Framework v4.0 - SportsPerformance</h1>
...
function setTab(t) {
    TAB = t;
    document.querySelectorAll('.tab').forEach(el => el.classList.toggle('active', el.innerText.includes(t.toUpperCase())));
...
async function load() {
    ...
    SNAPSHOT = d;
    athleteSelect.innerHTML = d.athlete360.map(a => `<option value="${escapeHtml(a.athlete)}">${escapeHtml(a.athlete)}</option>`).join('');
    renderTabs();
}
...
function renderTabs() {
    ...
    if(!item) return;
    sidebarContext.innerHTML = `<div class="sidebar-stat"><label>HCI ATUAL</label><b>${item.hci} (${item.level})</b></div>`;
...
function renderResumo(item) {
    leadsList.innerHTML = '<table><thead><tr><th>Atleta</th><th>Email</th><th>Ação</th></tr></thead><tbody>' +
        SNAPSHOT.leads.map((l,idx) => `<tr>
            <td><input id="ln_${idx}" value="${escapeHtml(l.athleteName)}"/></td>
            <td><input id="le_${idx}" value="${escapeHtml(l.athleteEmail)}"/></td>
            <td><button class="btn btn-blue" onclick="saveLead('${l.leadId}', ${idx})">Salvar</button> <button class="btn btn-red" onclick="delLead('${l.leadId}')">Deletar</button></td>
        </tr>`).join('') + '</tbody></table>';

    pendingISSF.innerHTML = SNAPSHOT.pendingISSF.length ? '...' : '<div style="color:var(--text-muted); text-align:center; padding:20px">Nenhum registro pendente.</div>';
    issfSessions.innerHTML = '<table><thead><tr><th>Evento</th><th>Total</th><th>Acao</th></tr></thead><tbody>' +
        (item.sessions || []).map(s => `<tr><td>${escapeHtml(s.evento)}</td><td>${s.total}</td><td><button class="btn btn-red">Deletar</button></td></tr>`).join('') + '</tbody></table>';
    issfResults.innerHTML = '<table><thead><tr><th>Atleta</th><th>Série</th><th>Tiros</th></tr></thead><tbody>' +
        SNAPSHOT.shotSeries.map(r => `<tr><td>${escapeHtml(r.atleta)}</td><td>${escapeHtml(r.serie)}</td><td>${escapeHtml(r.tiros)}</td></tr>`).join('') + '</tbody></table>';
    targetSessionsTable.innerHTML = '<table><thead><tr><th>Atleta</th><th>Evento</th><th>Zonas</th><th>Ações</th></tr></thead><tbody>' +
        SNAPSHOT.targetSessions.slice(0, 5).map(t => `<tr><td>${escapeHtml(t.athlete)}</td><td>${escapeHtml(t.event)}</td><td>${escapeHtml(t.zoneCounts)}</td><td><button class="btn btn-blue">Salvar</button> <button class="btn btn-blue">Lançar</button></td></tr>`).join('') + '</tbody></table>';
    prescriptionsList.innerHTML = '<table><thead><tr><th>Dia</th><th>Treino</th></tr></thead><tbody>' +
        SNAPSHOT.prescriptions.map(p => `<tr><td>${p.day}</td><td>${escapeHtml(p.trainingTitle)}</td></tr>`).join('') + '</tbody></table>';
}

async function addLead() {
    await fetch('/api/create-lead', {method:'POST', body:JSON.stringify({name:newLeadName.value, email:newLeadEmail.value})});
    load();
}
async function saveLead(id, idx) {
    const payload = {leadId:id, athleteName:document.getElementById('ln_'+idx).value, athleteEmail:document.getElementById('le_'+idx).value};
    await fetch('/api/update-lead', {method:'POST', body:JSON.stringify(payload)});
    load();
}
async function delLead(id) {
    if(confirm('Deletar lead?')) { await fetch('/api/delete-lead', {method:'POST', body:JSON.stringify({leadId:id})}); load(); }
}

async function sendExtra() {
    const payload = {day:exDay.value, trainingTitle:exTitle.value, block:exBlock.value, code:exCode.value};
    await fetch('/api/prescribe-extra', {method:'POST', body:JSON.stringify(payload)});
    load();
}

async function importTargetScan() {
    const payload = {athlete:athleteSelect.value, event:tsEvent.value, session:tsSession.value, prova:tsProva.value, text:tsText.value};
    await fetch('/api/import-targetscan', {method:'POST', body:JSON.stringify(payload)});
    load();
}
...
if __name__ == "__main__":
    ...
    print(f"SportsPerformance Admin v4.2 HTML ORIGINAL rodando em {url}")
